...
class window4_1:
    def __init__(self, info_dic, re=False, old2new=None):
        ...
        for F in (cpgxPage, cpxxPage, qtxxPage):
            ...
            self.frames[F] = frame
            frame.grid(row=0, column=0, sticky="nsew")  # 三个页面的位置都是 grid(row=0, column=0), 位置重叠，只有最上面的可见！！
...
class qtxxPage(tk.Frame):
    def __init__(self, parent, root):
        ...
        self.sjaqjyspxr1.set_position(row=15, column=0, rowspan=1, columnspan=2)
        self.sjaqjyspxr2.set_position(row=15, column=2, rowspan=1, columnspan=2)
        self.sjaqjyrq1.set_position(row=16, column=0, rowspan=1, columnspan=2)
        self.sjaqjyrq2.set_position(row=16, column=2, rowspan=1, columnspan=2)
        self.sjaqjyrq3.set_position(row=16, column=4, rowspan=1, columnspan=2)
        self.sjaqjybzpxr.set_position(row=17, column=0, rowspan=1, columnspan=2)
        self.kyj.set_position(row=17, column=2, rowspan=1, columnspan=2)
        if 'XS' in rzfw:
            self.xsbry1.set_position(row=18, column=0, rowspan=1, columnspan=2)
            self.xsbry2.set_position(row=18, column=2, rowspan=1, columnspan=2)
